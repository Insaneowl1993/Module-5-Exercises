...
router.get('/filter', (req, res) => {
  ...
  // let filterGender = req.query.letter;
  let matchingFriends = [...friends]

  // changed the way its filtering so you can return whatever query you want for more dynamic filtering

  for (let key in req.query) {
    matchingFriends = matchingFriends.filter(
      (friend) => friend[key] == req.query[key]
    )
  }
  ...
  // if (filterGender) {
  //     matchingFriends = matchingFriends.filter(friend => friend.letter == filterGender);
  // }
...
router.get('/letter', (req, res) => {
  ...
  if (filterLetter) {
    let matchingFriends = myFriends.filter(
      (friend) => friend.letter == filterLetter
    )
    if (matchingFriends.length > 0) {
      ...
      res
        .status(404)
        .json({ error: 'No friends match this initial ' + filterLetter })
    }
  }
  ...
  // } else {
  //     // and an error response when there are no matches
  //     res.status(404).json({error: "No friends match this initial "+filterLetter})
  // }
})
...
router.get('/:id', (req, res) => {
  ...
  let friendsId = req.params.id // 'id' here will be a value matching anything after the / in the request path

  // Modify this function to find and return the friend matching the given ID, or a 404 if not found
  let friends = friends.find((friends) => friends.id == parseInt(friendsId))
  friends
    ? res.status(200).json({ result: friends })
    : res.status(404).json({
        result: 'User ${friendId} not found',
      })
  // Modify this response with the matched friend, or a 404 if not found
  res.json({ result: 'Finding friend with ID ' + friendsId })
})
...
router.put('/:id', (req, res) => {
  ...
  friends = friends.find((friend) => friend.id == parseInt(friendId))
